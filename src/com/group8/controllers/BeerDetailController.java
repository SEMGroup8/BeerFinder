...
/**
 * Created by AnkanX on 15-10-27. TODO fix nice detail layout and images
 */
public class BeerDetailController extends BaseController implements Initializable {
	...
	boolean justRanked = false; // I'm using this to keep the rank as the user
								// just ranked the beer so he can know he ranked
								// the beer
	public Label added;
	@FXML
	...
	public void addToFavourite(ActionEvent event) throws IOException {
		if (UserData.userInstance != null) {
			String sqlQuery = "insert into favourites values(" + BeerData.selectedBeer.getId() + ", "
	...
	public void addToPub(ActionEvent event) throws IOException {
		if (UserData.userInstance != null) {
			if (UserData.userInstance.get_isPub()) {
				...
				addBeerToPub.setOnAction(new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent event) {

						String query = "Insert into beerInPub values(" + UserData.userInstance.get_pubId() + ", "
								+ BeerData.selectedBeer.getId() + ", " + Float.parseFloat(price.getText()) + ", 1)";
	...
	public boolean notRankedYet() {
		String selectQuery = "Select * from beerRank where userID = '" + UserData.userInstance.get_id()
